.ifndef JOYSTICK_SEND_TIMER
	...
	ADDI $sp, $sp, 4
	JR $ra
    .end setup_send_timer
    
     .ent send_timer_delay
	send_timer_delay:
	ADDI $sp, $sp, -12
	SW $ra, 0($sp)
	SW $t0, 4($sp)
	...
	ADDI $sp, $sp, 12
	JR $ra
     .end send_timer_delay
     
.Text
 .ent send_timer_handler 
    send_timer_handler:
    DI
